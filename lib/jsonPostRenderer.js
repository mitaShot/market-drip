    ...
    return sections.map((section, index) => {
        ...
        if (heading) {
            ...
            if (section.impact_level) {
                const colors = { high: { bg: '#ffebee', text: '#c62828' }, medium: { bg: '#fff3e0', text: '#ef6c00' }, low: { bg: '#e8f5e9', text: '#2e7d32' } };
        ...
        switch (section.type) {
            ...
            case 'news':
                ...
                break;
            case 'outlook':
                if (section.bull_case) html += `<div class="outlook-bull" style="background:#e8f5e9;border-left:4px solid #4caf50;padding:16px;margin:16px 0;border-radius:0 8px 8px 0;">\n<h4 style="margin:0 0 8px 0;">${t.bull_case}</h4>\n<p style="margin:0;">${formatContent(section.bull_case)}</p>\n</div>\n`;
                if (section.bear_case) html += `<div class="outlook-bear" style="background:#ffebee;border-left:4px solid #f44336;padding:16px;margin:16px 0;border-radius:0 8px 8px 0;">\n<h4 style="margin:0 0 8px 0;">${t.bear_case}</h4>\n<p style="margin:0;">${formatContent(section.bear_case)}</p>\n</div>\n`;
                if (section.strategy) html += `<div class="outlook-strategy" style="background:#fff3e0;border-left:4px solid #ff9800;padding:16px;margin:16px 0;border-radius:0 8px 8px 0;">\n<h4 style="margin:0 0 8px 0;">${t.strategy}</h4>\n<p style="margin:0;">${formatContent(section.strategy)}</p>\n</div>\n`;
                break;
            default:
                if (section.content) html += `<div class="section-content">${formatContent(section.content)}</div>\n`;
        }

        // 두 번째 섹션 뒤에 이미지 삽입 (chart 우선, 없으면 inner fallback)
        if (index === 1) {
            if (jsonData.chart) {
                const chart = jsonData.chart;
                html += `
                <figure class="post-chart-wrapper" style="margin: 28px 0; background: #fff; border-radius: 12px; overflow: hidden; border: 1px solid #ebf0f5; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    <img src="/posts/img/${chart.chart_filename}" 
                         alt="${escapeHtml(chart.image_alt || '')}" 
                         style="width:100%; height:auto; display:block;" 
                         referrerpolicy="no-referrer" />
                    <figcaption style="padding: 20px; background: #fcfdfe; border-top: 1px solid #ebf0f5;">
                        <div style="font-weight: 700; color: #1a202c; margin-bottom: 12px; font-size: 1.1rem; line-height: 1.4;">
                            ${escapeHtml(chart.image_caption || '')}
                        </div>
                        
                        ${chart.analysis ? `
                        <div style="font-size: 0.92rem; color: #4a5568; line-height: 1.6; background: #fff; padding: 14px; border-radius: 8px; border: 1px dashed #cbd5e0;">
                            ${formatContent(chart.analysis)}
                        </div>` : ''}
                    </figcaption>
                </figure>\n`;
            } else if (postId) {
...
function renderKeyStatistics(stats, t) {
    ...
    // N/A 값이 하나라도 있으면 표를 출력하지 않음
    const hasNA = Object.values(stats).some(val =>
