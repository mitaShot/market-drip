...
function renderClaims(claims, t, state = { linkCount: 0, seenUrls: new Set() }) {
    ...
    claims.forEach(item => {
        const text = item.what || item.claim || '';
        ...
        // SEO: Global duplicate check & Limit to 10 inline links
        if (url && !state.seenUrls.has(url) && state.linkCount < 10) {
...
function renderFaq(faq, t) {
    ...
    faq.forEach(item => {
        ...
        html += '<h3 style="font-size:1.1rem; font-weight:700; color:#1e293b; margin:0 0 8px 0; display:flex; gap:8px;"><span>Q.</span> ' + escapeHtml(item.question) + '</h3>';
            ...
            case 'news':
                ...
                if (section.type === 'news' && section.sources?.length > 0) {
                    ...
                    section.sources.forEach(source => {
                        sectionHtml += '<a href="' + escapeHtml(source.url) + '" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;padding:8px 14px;background:#fff;border:1px solid #ddd;border-radius:20px;text-decoration:none;color:#0066cc;font-size:14px;transition:all 0.2s;">' + escapeHtml(source.name) + ' <span style="margin-left:6px;">↗</span></a>';
                    });
                    sectionHtml += '</div></div>';
                }
                break;
...
export function parseJsonPost(jsonData, id, lang) {
    ...
    jsonData._renderState = { linkCount: 0, seenUrls: new Set() };
